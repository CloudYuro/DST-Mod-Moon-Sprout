...
local SLEEPER_NO_TAGS = { "player", "playerghost", "epic", "lunar_aligned", "INLIMBO" }
...
local function IsValidEnemy(target)
    -- 排除玩家和友好单位
    if target:HasTag("player") or target:HasTag("lunar_aligned") then
        return false
    end
...
local function RetargetFn(inst)
    ...
    if current_target ~= nil and IsValidEnemy(current_target) then
    ...
    for _, ent in ipairs(TheSim:FindEntities(x, y, z, 15)) do
        if IsValidEnemy(ent) then
            local dist_sq = ent:GetDistanceSqToPoint(x, y, z)
...
local function KeepTargetFn(inst, target)
    -- 只保持对有效敌人的仇恨
    return IsValidEnemy(target) and not inst.sg:HasStateTag("invisible")
end
...
local function fn()
	...
	inst:ListenForEvent("timerdone", function(inst, data)
		if data.name == "expire" then
			local fx_finish = SpawnPrefab("wormwood_lunar_transformation_finish")
			fx_finish.Transform:SetPosition(inst.Transform:GetWorldPosition())
			inst:Remove() 
		end
	end)
...
return Prefab("wormwood_lunar_grazer", fn, assets, prefabs),
