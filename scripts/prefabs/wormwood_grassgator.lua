...
local function OnHealthDelta(inst)
    if inst.components.health:IsHurt() then
        inst.components.health:StartRegen(10, 2.5)
    else
...
local function OnTimerDone(inst, data)
    ...
    if data.name == "planarentity" then
        if inst.components.planardamage then
            inst.components.planardamage:Remove()
        end
        if inst.components.planarentity then
            inst.components.planarentity:Remove()
        end
...
local function ShouldEat(inst)
    if inst.components.container then
        ...
        if item and item.prefab == "moon_tree_blossom_charged" then
            ...
            if inst.components.timer:TimerExists("planarentity") then
                inst.components.timer:SetTimeLeft("planarentity", inst.components.timer:GetTimeLeft("planarentity") + stacksize * TUNING.TOTAL_DAY_TIME)
            end
            inst.components.timer:StartTimer("planarentity", TUNING.TOTAL_DAY_TIME)
